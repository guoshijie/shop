...
$sql="select count(*) from `goods` where username='".$username."'&&type=1";
$rs=mysql_query($sql);
...
$offset=$pagesize*($page - 1);//计算记录偏移量
    ...
    echo "共".$pages."页(".$page."/".$pages.")&nbsp;";
	  if($page>1){
	  	echo "<a href='have.php?page=".$first."&ns=".$ns."'>首页</a>";
	  	echo "<a href='have.php?page=".$pre."&ns=".$ns."'>&nbsp;上一页</a>";
	  }
	  
	  for($i=1;$i<$page;$i++)
	  	  echo "<a href='have.php?page=".$i."&ns=".$ns."'>[".$i."]</a>";
	      echo "<font color='red'>[".$page."]</font>";
	  for($i=$page+1;$i<=$pages;$i++)
	  	 echo "<a href='have.php?page=".$i."&ns=".$ns."'>[".$i."]</a>";  
	  
	  if($page<$pages){
	  	echo "<a href='have.php?page=".$next."&ns=".$ns."'>下一页</a>";
	  	echo "<a href='have.php?page=".$last."&ns=".$ns."'>&nbsp;尾页</a>";
	  }
...
<table width="100%" height="81" border="0" cellspacing="20" class='zt2' >
       ...
       return $str;
           ...
           $sql="select * from `goods` where username='".$username."'&&type=1 order by fabutime desc limit $offset,$pagesize ";
